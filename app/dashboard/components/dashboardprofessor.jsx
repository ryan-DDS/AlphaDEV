...
// Esta é a fábrica que cria a tela de Professor (o mestre das aulas!).
  ...
  return (
    ...
    <div className="p-8 bg-gray-900 min-h-screen text-white">
      ...
      <div className="flex justify-between items-center mb-8 border-b border-white/10 pb-4">
        <div>
          <h1 className="text-3xl font-bold text-violet-600">Painel do Professor</h1>
          <p className="text-white/50 text-sm mt-1">Bem-vindo, Professor</p>
        </div>
        <button
        ...
        <div className="p-6 bg-gray-800/40 border border-white/5 rounded-xl backdrop-blur-sm flex flex-col h-[240px]">
          ...
          <div className="text-xs text-white/70 space-y-3 overflow-y-auto flex-1 pr-1">
            <p className="border-l-2 border-violet-500/50 pl-2 py-0.5">
              <strong className="text-violet-400 block mb-0.5">Cadastro de Alunos</strong>
              Como professor, você está autorizado apenas a criar e gerenciar perfis de "aluno".
            </p>
            <p className="border-l-2 border-blue-500/50 pl-2 py-0.5">
              <strong className="text-blue-400 block mb-0.5">Gestão de Aulas</strong>
              Alunos cadastrados recebem acesso imediato. Em caso de problemas com outros perfis, contate o administrador.
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-gray-800/40 border border-white/5 rounded-xl p-6 h-fit">
          ...
          <form onSubmit={handleSalvarUsuario} className="space-y-4">
            ...
            {/* Select para escolher o Cargo (Perfil) - Desabilitado para professores, pois só podem cadastrar alunos */}
            <div>
              <label className="block text-xs text-white/50 mb-1 uppercase font-semibold">
                Perfil
              </label>
              <select
                value={perfil}
                disabled
                className="w-full bg-gray-800/50 border border-white/10 rounded-lg p-2.5 text-sm focus:border-violet-500 outline-none text-white/50 cursor-not-allowed"
              >
              ...
              <p className="text-[11px] text-white/40 mt-1">
                Professores só podem cadastrar novos usuários como "aluno".
              </p>
            </div>
        ...
        {/* PARTE 2: A TABELA DE VISUALIZAÇÃO DOS USUÁRIOS (Sem botão de Excluir para os Professores) */}
                  ...
                  {Array.isArray(usuariosFiltrados) && usuariosFiltrados.length > 0 ? (
                    usuariosFiltrados.map((u) => (
                      ...
                      >
                        ...
                        {/* Botão de Editar (Professor não pode excluir ninguém!) */}
                        <td className="py-3 px-4 text-right">
                          <button
                            onClick={() => handleIniciarEdicao(u)}
                            className="text-xs cursor-pointer bg-white/5 hover:bg-violet-600/30 border border-white/10 hover:border-violet-500/50 px-3 py-1.5 rounded transition-all font-semibold text-violet-400"
                          >
                            Editar
                          </button>
                        </td>
